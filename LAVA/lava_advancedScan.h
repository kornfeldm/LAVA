...
#include "lava_log.h";
    ...
    int scanFile(std::string filePath, struct cl_engine* engine, struct cl_scan_options options, bool scanningDirectory) {
        const char* virname;
    ...
    void iterateDirectory(std::string directory, struct cl_engine* engine,
        struct cl_scan_options options, bool clean, int viruses_found)
    {
        ...
        if (dir != NULL)
        {
            ...
            while (item != NULL)
            {
                ...
                if (item->d_type == DT_DIR)
                {
                    ...
                    iterateDirectory(newDirectory, engine, options, clean, viruses_found);
                ...
                if (item->d_type == DT_REG)
                {
                    ...
                    if (scanFile(filePath, engine, options, true) == CL_VIRUS) {
    ...
    bool scanDirectory(std::string dirPath, struct cl_engine* engine, struct cl_scan_options options) {
        ...
        std::string start_time = get_time();
        int viruses_found = 0;
        iterateDirectory(dirPath, engine, options, clean, viruses_found);//call recursive helper funciton
        std::string finish_time = get_time();
